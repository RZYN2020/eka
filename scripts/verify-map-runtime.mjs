...
try {
	...
	assert(
		hoveredMarkerState.every(
			({ transform }) => transform.includes('translate3d') && !transform.includes('scale'),
		),
	...
	const segmentState = await page.evaluate(() => {
		...
		return {
			...
			markersStable: [...document.querySelectorAll('.leaflet-marker-icon')].every(
				(marker) =>
	...
	assert(await page.$eval('#journeyTimeline', (timeline) => timeline.hidden), 'Escape 应退出人生轨迹');
	...
	await reducedPage.setViewport({ width: 1280, height: 800, deviceScaleFactor: 1 });
	await reducedPage.emulateMediaFeatures([{ name: 'prefers-reduced-motion', value: 'reduce' }]);
	await reducedPage.goto(`${server.origin}/map/`, { waitUntil: 'domcontentloaded' });
	await reducedPage.waitForSelector('.leaflet-marker-icon', { timeout: 15_000 });
	await reducedPage.click('#btnJourney');
	await reducedPage.waitForFunction(() => document.getElementById('journeyCity')?.textContent === '庆阳');
	await reducedPage.click('#btnJourneyPlay');
	await reducedPage.click('#btnJourneyNext');
	await reducedPage.waitForFunction(() => document.getElementById('journeyCity')?.textContent === '西安');
	const reducedPosition = await reducedPage.$eval('.journey-avatar', (avatar) => {
		const rect = avatar.getBoundingClientRect();
		return { left: rect.left, top: rect.top };
	});
	await new Promise((resolve) => setTimeout(resolve, 100));
	assert.deepEqual(
		await reducedPage.$eval('.journey-avatar', (avatar) => {
			const rect = avatar.getBoundingClientRect();
			return { left: rect.left, top: rect.top };
		}),
		reducedPosition,
		'减少动态效果时轨迹头像应直接到达终点，不执行插值移动',
	);
	assert.equal(
		await reducedPage.evaluate(
			() =>
				document
					...
					.filter(
						(animation) =>
							typeof animation.animationName === 'string' && animation.animationName.startsWith('journey-'),
					).length,
		),
		0,
		'减少动态效果时不应运行轨迹 CSS 动画',
	);
	await reducedPage.close();
	console.log(
		`Map runtime verification passed (${markerLayout.markers.length} markers, zoom ${initialZoom} → ${segmentState.zoom}).`,
	);
} finally {
	await browser?.close();
	await server.close();
